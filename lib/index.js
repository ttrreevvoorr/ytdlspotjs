...
console.log("Downloads songs via Youtube found from track names in a Spotify album or playlist URL\n");

rl.question("\x1b[1mSpotify Playlist or Album URL: \x1b[0m", spotifyUrl => {
  let url = spotifyUrl;

  rl.question(`\x1b[1mDownload path: (${path.resolve(".")}${slash}${dir}) \x1b[0m`, desiredOut => {
    if (desiredOut) {
      dir = desiredOut;
    }
