...
module.exports = {
  ...
  playListGet: async url => {
    ...
    try {
      ...
      console.log("\x1b[31m%s\x1b[0m", "An error occured while trying to get information from the playlist. Please restart:\n" + e);
    }
  },
...
async function _validateURL(url) {
  if (!url || typeof url !== "string") {
    console.log("\x1b[31m%s\x1b[0m", "You did not specify the URL of Spotify! Please restart");
  }
  ...
  if (regexp.test(url)) {
    try {
      ...
      if (!supportedTypes.includes(parsedURL.type)) {
        console.log("\x1b[31m%s\x1b[0m", "The specified URL is not of a valid type. Please restart. Valid types: " + supportedTypes.join(", "));
      }
      return true;
    } catch (e) {
